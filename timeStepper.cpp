...
timeStepper::timeStepper(elasticPlate &m_plate, double m_viscus)
{
	plate = &m_plate;
    ...
    elasticJacobian.setZero(total_dof, total_dof);
...
void timeStepper::addElasticJacobian(int ind1, int ind2, double p)
{
	elasticJacobian(ind1, ind2) = elasticJacobian(ind1, ind2) + p;
}
...
void timeStepper::clearElasticJacobian()
{
	elasticJacobian.setZero(total_dof, total_dof);
}

void timeStepper::addDampingForce()
{
	//elasticJacobian.setZero(total_dof, total_dof);

	dampingForce = viscus * elasticJacobian * plate->u;

	for (int i = 0; i < total_dof; i++)
	{
		addForce(i, dampingForce(i));
	}
}

void timeStepper::addDampingJacobian()
{
	for (int i = 0; i < total_dof; i++)
	{
		for (int j = 0; j < total_dof; j++)
		{
			addJacobian(i, j, viscus * elasticJacobian(i, j) / plate->dt);
		}
	}
}
