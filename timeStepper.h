...
public:
	...
	timeStepper(elasticPlate &m_plate, double m_viscus);
	~timeStepper();
    ...
    VectorXd dampingForce;

	void setZero();
	void addForce(int ind, double p);
	void addJacobian(int ind1, int ind2, double p);
    void addElasticJacobian(int ind1, int ind2, double p);
	void integrator();

	void first_time_PARDISO_setup();

    void addDampingForce();
    void addDampingJacobian();

    void clearElasticJacobian();
